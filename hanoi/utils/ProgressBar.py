...
class ProgressBar():
    def __init__(self, total_epochs, total_batches, statistics_name):
    ...
    def new_epoch_begin(self):
        ...
        self.last_time_s = time.time_ns() / 1e9
        self.epoch_start_time_s = self.last_time_s
        
        self.print_line()
        
    def new_batch_done(self, statistics_val):
        self.current_batch += 1
        
        assert self.current_batch <= self.total_batches, "'current_batch' should be smaller than 'total_batches'"
        
        curr_time_s = time.time_ns() / 1e9
        
        if (curr_time_s - self.last_time_s) * 10 < self.per_batch_speed:
            self.per_batch_speed = (curr_time_s - self.last_time_s)
        else:
            self.per_batch_speed = self.per_batch_speed * (self.current_batch - 1) / self.current_batch + \
    ...
    def epoch_ends(self, statistics_val):
        self.print_line(statistics_val = statistics_val)
        print("")
