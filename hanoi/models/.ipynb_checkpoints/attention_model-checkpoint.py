...
class VisAttnModel(nn.Module):
    def __init__(self, num_classes, feature_dim = 64, attn_dim = 32):
        ...
        self.attn_layer = nn.MultiheadAttention(embed_dim = self.attn_dim, num_heads = 8)
...
if __name__ == "__main__":
    ...
    tr_dataset = HanoiDataset(disk_size_candidates = [3, 5], num_digits = 5, dataset_size = 20000)
    train_loader = DataLoader(tr_dataset, batch_size = batch_size, shuffle = True)

    ts_dataset = HanoiDataset(disk_size_candidates = [5], num_digits = 5, dataset_size = 10000)
